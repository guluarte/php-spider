...
$seed = 'http://lolzbook.com/';

// We want to allow all subdomains of dmoz.org
$allowSubDomains = false;

// Create spider
$spider = new Spider($seed, 'lolzbook-18sep');
...
$spider->addDiscoverer(new XPathExpressionDiscoverer("//a[contains(@href,'page')]"));
$spider->addDiscoverer(new XPathExpressionDiscoverer("//h2[@class='entry-title']//a"));

// Let's tell the spider to save all found resources on the filesystem
$spider->setPersistenceHandler(
	new \VDB\Spider\PersistenceHandler\JsonInfo(__DIR__ . '/results')
	);
...
foreach ($downloaded as $resource) {
	...
	try {
		...
		$imgNode = $resource->getCrawler()->filterXpath("//img[contains(@class,'size-full')]");
		$articleClasses = $resource->getCrawler()->filterXpath("//article[contains(@class,'post')]");
	} catch(\Exception $e) {
	...
	if ( is_object( $articleClasses )) {
		...
		foreach ($clases as $class) {
			if ( strstr($class, "category")) {
				$categories[] = str_replace("category-", null, $class);
			}
		}
	}
	//category-cute category-funny category-random
	if ( !strstr($title, "Lolz Book") && is_object($imgNode) && is_object( $articleClasses ) ) {
		$data = array(
			'title' => $title,
