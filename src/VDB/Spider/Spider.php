...
class Spider
{
    ...
    private function loadQueue() {
        @mkdir("./results/".$this->spiderId);
        $this->queueFileName = "./results/".$this->spiderId."/queue.json";
        $this->queueFile = "./results/".$this->spiderId."/str-queue.json";
        touch($this->queueFile);
        $this->fpQueue = fopen($this->queueFile, 'r');

        $this->alreadySeenUrisFile = "./results/".$this->spiderId."/alreadyseen.json";
        if ( file_exists(  $this->queueFileName) ) {
            $queeueContents = file_get_contents(  $this->queueFileName);
            $urlList = json_decode($queeueContents, true);
            foreach ($urlList as $uri) {
                echo "Loading Uri:".$uri."\n";
                $UriObjt = new Http($uri);
                //array_push($this->traversalQueue, $UriObjt);
            }
            
        }
        if ( file_exists($this->alreadySeenUrisFile)) {
            $areadySeenJson = file_get_contents($this->alreadySeenUrisFile);
            $this->crawled = json_decode($areadySeenJson, true);
        }
        if (is_array($this->crawled)) {
            foreach ($this->crawled as $uri => $seen) {
                $this->alreadySeenUris[$uri] = 0;
            }
        }
    ...
    private function saveQueue() {
        $urlList = array();
        foreach ($this->traversalQueue as $filterableUri) {
            //echo "Saving Uri: ".$filterableUri."\n";
            $urlList[] = $filterableUri->toString();
        }
        
        file_put_contents($this->queueFileName, json_encode($urlList));
        file_put_contents($this->alreadySeenUrisFile, json_encode($this->crawled));
    }
    /**
    ...
    {
        if (!feof($this->fpQueue)) {
            $url = trim( fgets($this->fpQueue));
            if ( substr($url, 0, 7) == "http://") {
                echo "[$url]\n";
                return new Http($url);
            } else {
               $url = rtrim($this->seed->normalize()->toString(), '/') . $url;
               echo "[$url]\n";
               return new Http($url);
           } 
    ...
    {
        while (!feof($this->fpQueue)) {
            $this->alreadySeenUris = array();
            $this->traversalQueue = array();
            /** @var $currentUri Uri */
            $currentUri = $this->getNextUriFromQueue();

            if (isset($this->crawled[$currentUri->normalize()->toString()]) ) {
                if ($this->crawled[$currentUri->normalize()->toString()] == 1) {
                    echo "YA SE HA VISITADO: ".$currentUri->normalize()->toString()."\n";
                    continue;
                }
            ...
            echo "Crawling: ".$currentUri->normalize()->toString()."\n";
            // Fetch the document
            if (!$resource = $this->fetchResource($currentUri)) {
                continue;
            }

            $this->crawled[$currentUri->normalize()->toString()] = true;
            $this->saveQueue();
            $this->dispatch(
            ...
            if (!isset($this->alreadySeenUris[$currentUri->toString()])) {
                $this->alreadySeenUris[$currentUri->toString()] = 0;
            }
            ...
            foreach ($discoveredUris as $uri) {
                ...
                if ($this->matchesPrefetchFilter($uri)) {
                    ...
                    echo "Encontrado ".$uri->toString()."\n";
                    $this->addUriToQueue($uri->normalize()->toString());
                }
                $this->alreadySeenUris[$uri->toString()] = $nextLevel;
            }
        }
    }
    private function addUriToQueue($url) {
        file_put_contents($this->queueFile, $url . PHP_EOL, FILE_APPEND | LOCK_EX);
    }
    /**
    ...
    protected function fetchResource(UriInterface $uri)
    {
        ...
        try {
            ...
            if (!isset($this->alreadySeenUris[$uri->toString()])) {
                $this->alreadySeenUris[$uri->toString()] = 0;
            }
    ...
    private function setSeed($uri)
    {
        ...
        $this->addUriToQueue($this->seed->normalize()->toString());
